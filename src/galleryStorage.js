...
const { DATABASE_NAME, STORE_NAME, DATABASE_VERSION, BUCKET_NAME } =
  GALLERY_CONFIG;
...
const getSupabaseConfig = () => {
  ...
  if (
    envUrl &&
    envKey &&
    !isPlaceholderValue(envUrl) &&
    !isPlaceholderValue(envKey)
  ) {
  ...
  if (
    runtimeUrl &&
    runtimeKey &&
    !isPlaceholderValue(runtimeUrl) &&
    !isPlaceholderValue(runtimeKey)
  ) {
    ...
    if (!isPublishableKey && !isJwtKey) {
      console.warn(
        "⚠️ Supabase key format appears invalid. Expected a JWT or Supabase publishable key.",
      );
      console.warn(
        "⚠️ Current key format:",
        runtimeKey.substring(0, 20) + "...",
      );
    } else {
...
if (supabase) {
  ...
  console.warn(
    "⚠️ Supabase not configured - local IndexedDB fallback is enabled",
    {
      source: config.source,
      hasUrl: !!config.url,
      hasKey: !!config.key,
...
const fallbackToLocalStorage = async () => {
  ...
  try {
    ...
    const images = await requestAsPromise(
...
export const getGalleryImages = async () => {
  ...
  try {
    ...
    const images = await Promise.all(
      data.map(async (image) => {
        try {
          if (!image.image_url) {
            console.warn(
              `⚠️ Skipping record ${image.id} because it has no image URL`,
            );
            return null;
          }
          ...
          if (!response.ok) {
            console.warn(
              `⚠️ Failed to fetch image ${image.id}:`,
              response.statusText,
            );
            return null;
          }
    ...
    console.log(
      `✅ Successfully loaded ${validImages.length} images from Supabase`,
    );
    return validImages;
  } catch (error) {
    console.error(
      "❌ Shared gallery unavailable. Check the Supabase table, bucket policies, and project URL.",
      error,
    );
    throw new Error(
...
export const saveGalleryImage = async (blob, metadata) => {
  if (!supabase) {
    ...
    const existingImages = await requestAsPromise(
      readTransaction.objectStore(STORE_NAME).getAll(),
    );
    database.close();

    const duplicate = existingImages.find(
      (image) => image.imageHash === metadata.imageHash,
    );
  ...
  try {
    ...
    if (uploadResult.error) {
      const errorMessage =
        uploadResult.error.message || "Unknown Supabase storage error";
    ...
    throw new Error(
      "Supabase upload failed. Check the bucket policies, scans table, and project permissions.",
    );
  }
};
...
export const removeGalleryImage = async (id) => {
  ...
  try {
    ...
    if (!image) {
      console.warn(
        "⚠️ No scan row found for delete request. Nothing to remove from Supabase.",
        { id },
      );
      return;
    }
    ...
    if (storageCandidates.length > 0) {
      ...
      if (storageError) {
        console.warn(
          "⚠️ Error deleting from storage (non-critical):",
          storageError,
        );
      }
    }
