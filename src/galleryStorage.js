...
const isPlaceholderValue = (value) => {
  ...
  return trimmedValue === '' || trimmedValue.includes('YOUR_') || trimmedValue.includes('your-') || trimmedValue.includes('your ') || trimmedValue === 'your-anon-key-here'
}
...
const supabase = config.url && config.key ? createClient(config.url, config.key, {
  auth: {
...
if (supabase) {
  ...
  supabase.from('scans').select('count', { count: 'exact', head: true }).then(({ count, error }) => {
    if (error) {
...
export const getGalleryImages = async () => {
  ...
  try {
    ...
    throw new Error('Supabase gallery is configured but unavailable. Please verify the scans table and plant-images bucket permissions.')
  }
}
